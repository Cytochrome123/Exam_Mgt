...
const AssignStudent = () => {
    ...
    const b = (studentId) => {
        details.selectedStudents.forEach(selected => {
            setBadge(prev => !prev)
        });
    }
console.log(details)
    ...
    return (
        <div>
            ...
            <Button variant='secondary' onClick={assign} >Add Selected Students</Button>

            {details.students.map(student => (

            <Card style={{ width: '18rem' }} className="mb-2" bg='secondary' >
                <Card.Body>
                    <Card.Title>{student.firstName} {student.lastName} {badge ? <Badge bg='success'>///</Badge> : '' } </Card.Title>
